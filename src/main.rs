...
fn setup(mut commands: Commands, windows: Query<&mut Window>) {
    ...
    let white_keys_vec: Vec<(String, i32)> = white_keys
        .iter()
        .map(|(k, v)| (k.to_string(), *v))
        .collect();
    let black_keys_vec: Vec<(String, i32)> = black_keys
        .iter()
        .map(|(k, v)| (k.to_string(), *v))
        .collect();
    ...
    println!("White key width: {:?}", app_state);
...
fn main() {
    ...
    .add_systems(
        Startup,
        (setup, setup_piano, setup_audio_files, spawn_notes).chain(),
    );
    app.run();
}
